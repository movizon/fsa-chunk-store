...
export default class FSAChunkStore {
  constructor(chunkLength, opts = {}) {
    ...
    this.chunksDirPromise =
      (opts.files && opts.rootDir && this._getChunksDirHandle()) || this.storageDirPromise
  ...
  async _getDirectoryHandle(opts) {
    ...
    if (!this.directoryMap[path]) {
      ...
      this.directoryMap[path] = (await parent).getDirectoryHandle(
        ...
        return (async () => {
          try {
            ...
            await stream.write({
              ...
              data: buf.slice(target.from, target.to),
            })
            return null
          } catch (err) {
            return cb(err)
